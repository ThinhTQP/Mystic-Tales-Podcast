...
export async function callGraphQLQuery(
    { client, query, variables }: GraphQLApiHelperParams,
    title?: string
): Promise<MessResponse> {
    const mess_title = title || null;
    const request_variables = variables || {};
    try {
        const response: ApolloQueryResult<any> | FetchResult<any> = await client.query({
            query: gql`${query}`,
