...
{
    public class PodcastSubscriptionIncomeReleaseJob : BackgroundService
    {
        private readonly ConsulDistributedLockService _lockService;
        private readonly IBackgroundJobsConfig _jobsConfig;
        private readonly ILogger<PodcastSubscriptionIncomeReleaseJob> _logger;
        ...
        // Job configuration
        private BackgroundJob _jobConfig => _jobsConfig.PodcastSubscriptionIncomeReleaseJob;
        private CronExpression? _cronExpression;

        public PodcastSubscriptionIncomeReleaseJob(
            ConsulDistributedLockService lockService,
            IBackgroundJobsConfig jobsConfig,
            ILogger<PodcastSubscriptionIncomeReleaseJob> logger,
        ...
        public override Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    ...
                    nameof(PodcastSubscriptionIncomeReleaseJob),
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                _logger.LogInformation("Background job is disabled: {JobName}", nameof(PodcastSubscriptionIncomeReleaseJob));
                return;
            }
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogInformation("Background job cancelled: {JobName}", nameof(PodcastSubscriptionIncomeReleaseJob));
                    break;
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Unexpected error in background job execution loop: {JobName}", nameof(PodcastSubscriptionIncomeReleaseJob));
                    await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
                }
            }

            _logger.LogInformation("Background job stopped: {JobName}", nameof(PodcastSubscriptionIncomeReleaseJob));
        }
        ...
        private async Task ExecuteJobWithLockAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var lockOptions = new ConsulLockOptions
                {
                    ...
                    {
                        ["JobName"] = nameof(PodcastSubscriptionIncomeReleaseJob),
        ...
        private async Task ExecuteJobLogicAsync(string executionId, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogDebug("Starting podcast subscription income release: ExecutionId={ExecutionId}", executionId);
                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    await podcastSubscriptionService.PodcastSubscriptionIncomeReleaseAsync();
                }

                Console.WriteLine("\n ----Podcast subscription income release----");
                // in thời điểm hiện tại
                Console.WriteLine($"✅ Current time: {_dateHelper.GetNowByAppTimeZone()} \n");


                _logger.LogDebug("Podcast subscription income release successfully: ExecutionId={ExecutionId}", executionId);
            }
            catch (OperationCanceledException)
            {
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error updating Podcast subscription income release: ExecutionId={ExecutionId}",
                    executionId);
                throw;
            }
        }
