...
namespace SubscriptionService.BusinessLogic.Services.BackgroundServices.PodcastSubscriptionJob
{
    public class PodcastSubscriptionRegistrationRenewalJob : BackgroundService
    {
        private readonly ConsulDistributedLockService _lockService;
        private readonly IBackgroundJobsConfig _jobsConfig;
        private readonly ILogger<PodcastSubscriptionRegistrationRenewalJob> _logger;
        ...
        // Job configuration
        private BackgroundJob _jobConfig => _jobsConfig.PodcastSubscriptionRegistrationRenewalJob;
        private CronExpression? _cronExpression;

        public PodcastSubscriptionRegistrationRenewalJob(
            ConsulDistributedLockService lockService,
            IBackgroundJobsConfig jobsConfig,
            ILogger<PodcastSubscriptionRegistrationRenewalJob> logger,
        ...
        public override Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    ...
                    nameof(PodcastSubscriptionRegistrationRenewalJob),
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                _logger.LogInformation("Background job is disabled: {JobName}", nameof(PodcastSubscriptionRegistrationRenewalJob));
                return;
            }
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogInformation("Background job cancelled: {JobName}", nameof(PodcastSubscriptionRegistrationRenewalJob));
                    break;
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Unexpected error in background job execution loop: {JobName}", nameof(PodcastSubscriptionRegistrationRenewalJob));
                    await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
                }
            }

            _logger.LogInformation("Background job stopped: {JobName}", nameof(PodcastSubscriptionRegistrationRenewalJob));
        }
        ...
        private async Task ExecuteJobWithLockAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var lockOptions = new ConsulLockOptions
                {
                    ...
                    {
                        ["JobName"] = nameof(PodcastSubscriptionRegistrationRenewalJob),
        ...
        private async Task ExecuteJobLogicAsync(string executionId, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogDebug("Starting podcast subscription registration renewal: ExecutionId={ExecutionId}", executionId);
                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    await podcastSubscriptionService.PodcastSubscriptionRegistrationRenewalAsync();
                }

                Console.WriteLine("\n ----Podcast subscription registration renewal----");
                // in thời điểm hiện tại
                Console.WriteLine($"✅ Current time: {_dateHelper.GetNowByAppTimeZone()} \n");


                _logger.LogDebug("Podcast subscription registration renewal successfully: ExecutionId={ExecutionId}", executionId);
            }
            catch (OperationCanceledException)
            {
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error updating Podcast subscription registration renewal: ExecutionId={ExecutionId}",
                    executionId);
                throw;
            }
        }
