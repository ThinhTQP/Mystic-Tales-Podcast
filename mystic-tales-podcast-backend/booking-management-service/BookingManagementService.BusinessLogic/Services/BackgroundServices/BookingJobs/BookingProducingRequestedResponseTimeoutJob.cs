...
{
    public class BookingProducingRequestedResponseTimeoutJob : BackgroundService
    {
        private readonly ConsulDistributedLockService _lockService;
        private readonly IBackgroundJobsConfig _jobsConfig;
        private readonly ILogger<BookingProducingRequestedResponseTimeoutJob> _logger;
        ...
        // Job configuration
        private BackgroundJob _jobConfig => _jobsConfig.BookingProducingRequestedResponseTimeoutJob;
        private CronExpression? _cronExpression;

        public BookingProducingRequestedResponseTimeoutJob(
            ConsulDistributedLockService lockService,
            IBackgroundJobsConfig jobsConfig,
            ILogger<BookingProducingRequestedResponseTimeoutJob> logger,
        ...
        public override Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    ...
                    nameof(BookingProducingRequestedResponseTimeoutJob),
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                _logger.LogInformation("Background job is disabled: {JobName}", nameof(BookingProducingRequestedResponseTimeoutJob));
                return;
            }
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogInformation("Background job cancelled: {JobName}", nameof(BookingProducingRequestedResponseTimeoutJob));
                    break;
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Unexpected error in background job execution loop: {JobName}", nameof(BookingProducingRequestedResponseTimeoutJob));
                    await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
                }
            }

            _logger.LogInformation("Background job stopped: {JobName}", nameof(BookingProducingRequestedResponseTimeoutJob));
        }
        ...
        private async Task ExecuteJobWithLockAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var lockOptions = new ConsulLockOptions
                {
                    ...
                    {
                        ["JobName"] = nameof(BookingProducingRequestedResponseTimeoutJob),
        ...
        private async Task ExecuteJobLogicAsync(string executionId, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogDebug("Starting booking producing requested response timeout: ExecutionId={ExecutionId}", executionId);
                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    await bookingService.BookingProducingRequestedResponseTimeoutAsync();
                }

                Console.WriteLine("\n ----Booking producing requested response timeout----");
                // in thời điểm hiện tại
                Console.WriteLine($"✅ Current time: {_dateHelper.GetNowByAppTimeZone()} \n");


                _logger.LogDebug("Booking producing requested response timeout successfully: ExecutionId={ExecutionId}", executionId);
            }
            catch (OperationCanceledException)
            {
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error updating Booking producing requested response timeout: ExecutionId={ExecutionId}",
                    executionId);
                throw;
            }
        }
